...
public class TestIntegration {
	...
	void mustCreateNewAnimal() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/api/animal/create").content(json)
				...
				.andExpect(status().isCreated()).andExpect(MockMvcResultMatchers.jsonPath("$").hasJsonPath())
				...
				.andExpect(MockMvcResultMatchers.jsonPath("$.conservationState")
						.value(animalDto.conservationState().name()))
				...
				.andExpect(MockMvcResultMatchers.jsonPath("$.canCauseSeriousAccident")
						.value(animalDto.canCauseSeriousAccident()))
	...
	void mustNotCreateAnimalWhenCnnDoesNotRecognizeLabel() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/api/animal/create").content(json)
				.contentType(MediaType.APPLICATION_JSON_VALUE).header(headerName, headerValues))
				.andExpect(status().isBadRequest()).andExpect(MockMvcResultMatchers.jsonPath("$.date").isNotEmpty())
	...
	void mustNotBreedArachnidTypeAnimalWhenDentitionIsDefined() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/api/animal/create").content(json)
				...
				.andExpect(status().isBadRequest()).andExpect(MockMvcResultMatchers.jsonPath("$.date").isNotEmpty())
				.andExpect(
						MockMvcResultMatchers.jsonPath("$.message").value("Aracnídeo não pode ter dentição definida"))
				.andExpect(MockMvcResultMatchers.jsonPath("$.detail").value("uri=/api/animal/create"));
	}
	...
	void mustThrowNotFoundExceptionWhenIdDoesNotExistInEdit() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.put("/api/animal/update").content(json)
				.contentType(MediaType.APPLICATION_JSON_VALUE).header(headerName, headerValues))
				.andExpect(status().isNotFound()).andExpect(MockMvcResultMatchers.jsonPath("$.date").isNotEmpty())
	...
	void mustReturnLabelsAvailableForRegistration() throws Exception {
		...
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/animal/find/labels/available/").header(headerName, headerValues))
				.andExpect(status().isOk()).andExpect(MockMvcResultMatchers.jsonPath("$").hasJsonPath())
	...
	void mustReturnAnimalShort() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/api/animal/list?page=0&size=30&sort=id,asc").header(headerName,
				headerValues)).andExpect(status().isOk()).andExpect(MockMvcResultMatchers.jsonPath("$").hasJsonPath())
				.andExpect(MockMvcResultMatchers.jsonPath("$").isNotEmpty())
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/api/animal/list").header(headerName, headerValues))
				.andExpect(status().isOk()).andExpect(MockMvcResultMatchers.jsonPath("$").hasJsonPath())
				.andExpect(MockMvcResultMatchers.jsonPath("$").isNotEmpty())
				.andExpect(MockMvcResultMatchers.jsonPath("$.content[0].id").value(id))
				.andExpect(MockMvcResultMatchers.jsonPath("$.content[0].label").value(label))
	...
	void whenPassingDescriptionItMustReturnAnimal() throws Exception {
		...
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/animal/find/description/" + genre + "?page=0&size=15&sort=label")
